...
public abstract class UiBasedBackgroundTask<Result> {
    ...
    @Nullable private UiCallback<Result> callback;
    @Nullable private Result awaitedResult;

    private volatile boolean isCanceled = false;

    /**
     * @param failedResult The result to return if the task fails (throws an exception or returns
     *     null).
     */
    ...
    /**
     * @param callback If null, any results which may arrive from a currently executing task will be
     *     stored until a new callback is set.
     */
    public void setCallback(@Nullable UiCallback<Result> callback) {
        synchronized (callbackLock) {
            ...
            // If we have an awaited result and are setting a new callback, publish the result
            // immediately.
    ...
    private void emitResultOnMainThread(
            @NonNull UiCallback<Result> callback, @NonNull Result result) {
     ...
     * @throws Exception If an Exception is thrown from this task's execution, it will be logged and
     *     the provided default Result will be returned.
     */
    @NonNull
