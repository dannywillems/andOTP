...
public class AuthenticationTask extends UiBasedBackgroundTask<Result> {
    ...
    /**
     * @param context Context to be used to query settings (the application Context will be used to
     *     avoid memory leaks).
     * @param isAuthUpgrade true if this is an authentication upgrade and new credentials should be
     *     saved, false if this is just confirmation.
     * @param existingAuthCredentials The existing hashed authentication credentials that we have
     *     stored.
     * @param plainPassword The plaintext user-entered password to check authentication with.
     */
    public AuthenticationTask(
            Context context,
            boolean isAuthUpgrade,
    ...
    private Result upgradeAuthentication() {
        ...
        if (!hashedPassword.equals(existingAuthCredentials)) return Result.failure();
        ...
        if (key == null) return Result.upgradeFailure();
        else return Result.success(key);
    }

    @NonNull
    private Result confirmAuthentication() {
        try {
            PBKDF2Credentials credentials =
                    EncryptionHelper.generatePBKDF2Credentials(
                            plainPassword, settings.getSalt(), settings.getIterations());
    ...
    public static class Result {
        @Nullable public final byte[] encryptionKey;
