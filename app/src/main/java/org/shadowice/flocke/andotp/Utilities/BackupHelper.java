...
public class BackupHelper {
    ...
    public static BackupFile backupFile(
            Context context, Uri backupLocationUri, Constants.BackupType type) {
        ...
        if (backupLocation != null) {
            ...
            if (backupFile.file == null) {
                backupFile.file =
                        backupLocation.createFile(
                                backupMimeType(type), BackupHelper.backupFilename(context, type));
            }
    ...
    public static String backupFilename(Context context, Constants.BackupType type) {
        ...
        switch (type) {
            case PLAIN_TEXT:
                if (settings.getIsAppendingDateTimeToBackups()) {
                    return String.format(
                            Constants.BACKUP_FILENAME_PLAIN_FORMAT, Tools.getDateTimeString());
                } else {
            ...
            case ENCRYPTED:
                if (settings.getIsAppendingDateTimeToBackups()) {
                    return String.format(
                            Constants.BACKUP_FILENAME_CRYPT_FORMAT, Tools.getDateTimeString());
                } else {
            ...
            case OPEN_PGP:
                if (settings.getIsAppendingDateTimeToBackups()) {
                    return String.format(
                            Constants.BACKUP_FILENAME_PGP_FORMAT, Tools.getDateTimeString());
                } else {
    ...
    public static boolean backupToFile(Context context, Uri uri, String password, String plain) {
        ...
        try {
            ...
            byte[] encrypted =
                    EncryptionHelper.encrypt(key, plain.getBytes(StandardCharsets.UTF_8));

            byte[] iterBytes = ByteBuffer.allocate(Constants.INT_LENGTH).putInt(iter).array();
            byte[] data =
                    new byte
                            [Constants.INT_LENGTH
                                    + Constants.ENCRYPTION_IV_LENGTH
                                    + encrypted.length];
